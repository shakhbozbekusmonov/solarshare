...
/**
 * Atomically marks a transaction as SUCCESS and order as PAID.
 *
 * Listing kWh is reserved at order creation time to avoid oversell race conditions.
 */
export async function completeOrder(transactionId: string): Promise<void> {
	await prisma.$transaction(async db => {
		...
		if (
			Number(transaction.order.listing.availableKwh) <= 0 &&
			transaction.order.listing.status !== 'SOLD'
		) {
...
/**
 * Atomically marks a transaction as FAILED, order as CANCELLED,
 * and restores the reserved kWh back to the listing.
 */
export async function cancelOrder(transactionId: string): Promise<void> {
	await prisma.$transaction(async db => {
		const tx = await db.transaction.findUnique({
			where: { id: transactionId },
			include: {
				order: { select: { id: true, requestedKwh: true, listingId: true } },
			},
		})
		...
		// Restore the kWh that was reserved at order creation
		await db.listing.update({
			where: { id: tx.order.listingId },
			data: { availableKwh: { increment: tx.order.requestedKwh } },
		})

		// Reactivate the listing if it was marked SOLD due to this reservation
